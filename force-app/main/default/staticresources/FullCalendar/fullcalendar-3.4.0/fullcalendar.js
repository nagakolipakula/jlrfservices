...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var timeText;
		var titleHtml;
		var tooltipTxt = htmlEscape(this.getEventTimeText(event, null, true)) + (event.title ? ' ' + htmlEscape(event.title) : '');
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							button = $( // type="button" so that it doesn't submit a form
								'<button type="button" class="slds-button slds-button--neutral slds-truncate ' + classes.join(' ') + '">' +
									innerHtml +
								'</button>'
								)
								.click(function(ev) {
