...
(function($$) {
  var isLtng = window.location.href.indexOf("ltn_app_id") !== -1;
  var VF = {isVF:true, isLtng:isLtng, oauthToken:"null", instanceId:window.name, targetOrigin:"*"}, internalClient = $$.client;
  var module = function() {
    function subscribe(event) {
      internalClient.subscribe(VF, event)
    }
    function unsubscribe(event) {
      internalClient.unsubscribe(VF, event)
    }
    function publish(event) {
      internalClient.publish(VF, event)
    }
    function resize(event) {
      internalClient.resize(VF, event)
    }
    return{subscribe:subscribe, unsubscribe:unsubscribe, publish:publish, resize:resize}
  }();
  $$.module("Sfdc.canvas.publisher", module);
  $$.module("Sfdc.canvas.client", null)
})(Sfdc.canvas);
